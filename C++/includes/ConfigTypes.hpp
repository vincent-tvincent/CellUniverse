...
public:
    ...
    float pca_shape_bg_degenerate_sigma_threshold = 1e-6f;
    float pca_shape_bg_degenerate_p99_fraction = 0.50f;
    bool pca_shape_outer_shell_shrink_enabled = false;
    float pca_shape_outer_shell_inner_scale = 0.50f;
    float pca_shape_outer_shell_min_outer_inner_ratio = 0.35f;
    float pca_shape_outer_shell_min_signal = 0.00005f;
    float pca_shape_outer_shell_shrink_step_fraction = 0.08f;
    float pca_shape_outer_shell_max_shrink_fraction = 0.35f;
    int pca_shape_outer_shell_sample_stride = 2;
    ...
    bool bbox_local_background_enabled = false;
    bool bbox_local_background_apply_to_thresholds = true;
    bool bbox_local_background_apply_to_synth_cost = false;
    float bbox_local_background_percentile = 0.50f;
    float bbox_local_background_cell_exclusion_scale = 1.15f;
    float bbox_local_background_blend_with_frame = 0.50f;
    int bbox_local_background_min_samples = 1000;
    float bbox_local_background_min_delta = -0.02f;
    float bbox_local_background_max_delta = 0.05f;
    float bbox_local_background_synth_margin = 0.01f;
    bool bbox_local_background_debug = false;
    bool bbox_local_background_burn_into_synth = false;
    float bbox_local_background_burn_cell_exclusion_scale = 1.15f;
    float bbox_local_background_burn_feather_radius = 12.0f;
    float bbox_local_background_burn_margin = 0.01f;
    float bbox_local_background_burn_global_fallback_weight = 0.05f;
    bool bbox_local_background_burn_debug = false;
    ...
    void explodeConfig(const YAML::Node& node) {
        ...
        if (node["pca_shape_bg_degenerate_sigma_threshold"]) pca_shape_bg_degenerate_sigma_threshold = node["pca_shape_bg_degenerate_sigma_threshold"].as<float>();
        if (node["pca_shape_bg_degenerate_p99_fraction"]) pca_shape_bg_degenerate_p99_fraction = node["pca_shape_bg_degenerate_p99_fraction"].as<float>();
        if (node["pca_shape_outer_shell_shrink_enabled"]) pca_shape_outer_shell_shrink_enabled = node["pca_shape_outer_shell_shrink_enabled"].as<bool>();
        if (node["pca_shape_outer_shell_inner_scale"]) pca_shape_outer_shell_inner_scale = node["pca_shape_outer_shell_inner_scale"].as<float>();
        if (node["pca_shape_outer_shell_min_outer_inner_ratio"]) pca_shape_outer_shell_min_outer_inner_ratio = node["pca_shape_outer_shell_min_outer_inner_ratio"].as<float>();
        if (node["pca_shape_outer_shell_min_signal"]) pca_shape_outer_shell_min_signal = node["pca_shape_outer_shell_min_signal"].as<float>();
        if (node["pca_shape_outer_shell_shrink_step_fraction"]) pca_shape_outer_shell_shrink_step_fraction = node["pca_shape_outer_shell_shrink_step_fraction"].as<float>();
        if (node["pca_shape_outer_shell_max_shrink_fraction"]) pca_shape_outer_shell_max_shrink_fraction = node["pca_shape_outer_shell_max_shrink_fraction"].as<float>();
        if (node["pca_shape_outer_shell_sample_stride"]) pca_shape_outer_shell_sample_stride = node["pca_shape_outer_shell_sample_stride"].as<int>();
        ...
        if (node["bbox_local_background_enabled"]) bbox_local_background_enabled = node["bbox_local_background_enabled"].as<bool>();
        if (node["bbox_local_background_apply_to_thresholds"]) bbox_local_background_apply_to_thresholds = node["bbox_local_background_apply_to_thresholds"].as<bool>();
        if (node["bbox_local_background_apply_to_synth_cost"]) bbox_local_background_apply_to_synth_cost = node["bbox_local_background_apply_to_synth_cost"].as<bool>();
        if (node["bbox_local_background_percentile"]) bbox_local_background_percentile = node["bbox_local_background_percentile"].as<float>();
        if (node["bbox_local_background_cell_exclusion_scale"]) bbox_local_background_cell_exclusion_scale = node["bbox_local_background_cell_exclusion_scale"].as<float>();
        if (node["bbox_local_background_blend_with_frame"]) bbox_local_background_blend_with_frame = node["bbox_local_background_blend_with_frame"].as<float>();
        if (node["bbox_local_background_min_samples"]) bbox_local_background_min_samples = node["bbox_local_background_min_samples"].as<int>();
        if (node["bbox_local_background_min_delta"]) bbox_local_background_min_delta = node["bbox_local_background_min_delta"].as<float>();
        if (node["bbox_local_background_max_delta"]) bbox_local_background_max_delta = node["bbox_local_background_max_delta"].as<float>();
        if (node["bbox_local_background_synth_margin"]) bbox_local_background_synth_margin = node["bbox_local_background_synth_margin"].as<float>();
        if (node["bbox_local_background_debug"]) bbox_local_background_debug = node["bbox_local_background_debug"].as<bool>();
        if (node["bbox_local_background_burn_into_synth"]) bbox_local_background_burn_into_synth = node["bbox_local_background_burn_into_synth"].as<bool>();
        if (node["bbox_local_background_burn_cell_exclusion_scale"]) bbox_local_background_burn_cell_exclusion_scale = node["bbox_local_background_burn_cell_exclusion_scale"].as<float>();
        if (node["bbox_local_background_burn_feather_radius"]) bbox_local_background_burn_feather_radius = node["bbox_local_background_burn_feather_radius"].as<float>();
        if (node["bbox_local_background_burn_margin"]) bbox_local_background_burn_margin = node["bbox_local_background_burn_margin"].as<float>();
        if (node["bbox_local_background_burn_global_fallback_weight"]) bbox_local_background_burn_global_fallback_weight = node["bbox_local_background_burn_global_fallback_weight"].as<float>();
        if (node["bbox_local_background_burn_debug"]) bbox_local_background_burn_debug = node["bbox_local_background_burn_debug"].as<bool>();
    ...
    void printConfig() const {
        ...
        std::cout << "pca_shape_use_local_bg_estimation: " << pca_shape_use_local_bg_estimation << '\n';
        std::cout << "pca_shape_bg_shell_inner_scale: " << pca_shape_bg_shell_inner_scale << '\n';
        std::cout << "pca_shape_bg_shell_outer_scale: " << pca_shape_bg_shell_outer_scale << '\n';
        std::cout << "pca_shape_bg_percentile: " << pca_shape_bg_percentile << '\n';
        std::cout << "pca_shape_bg_sigma_k: " << pca_shape_bg_sigma_k << '\n';
        std::cout << "pca_shape_bg_degenerate_sigma_threshold: " << pca_shape_bg_degenerate_sigma_threshold << '\n';
        std::cout << "pca_shape_bg_degenerate_p99_fraction: " << pca_shape_bg_degenerate_p99_fraction << '\n';
        std::cout << "pca_shape_outer_shell_shrink_enabled: " << pca_shape_outer_shell_shrink_enabled << '\n';
        std::cout << "pca_shape_outer_shell_inner_scale: " << pca_shape_outer_shell_inner_scale << '\n';
        std::cout << "pca_shape_outer_shell_min_outer_inner_ratio: " << pca_shape_outer_shell_min_outer_inner_ratio << '\n';
        std::cout << "pca_shape_outer_shell_min_signal: " << pca_shape_outer_shell_min_signal << '\n';
        std::cout << "pca_shape_outer_shell_shrink_step_fraction: " << pca_shape_outer_shell_shrink_step_fraction << '\n';
        std::cout << "pca_shape_outer_shell_max_shrink_fraction: " << pca_shape_outer_shell_max_shrink_fraction << '\n';
        std::cout << "pca_shape_outer_shell_sample_stride: " << pca_shape_outer_shell_sample_stride << '\n';
        ...
        std::cout << "bbox_local_background_enabled: " << bbox_local_background_enabled << '\n';
        std::cout << "bbox_local_background_apply_to_thresholds: " << bbox_local_background_apply_to_thresholds << '\n';
        std::cout << "bbox_local_background_apply_to_synth_cost: " << bbox_local_background_apply_to_synth_cost << '\n';
        std::cout << "bbox_local_background_percentile: " << bbox_local_background_percentile << '\n';
        std::cout << "bbox_local_background_cell_exclusion_scale: " << bbox_local_background_cell_exclusion_scale << '\n';
        std::cout << "bbox_local_background_blend_with_frame: " << bbox_local_background_blend_with_frame << '\n';
        std::cout << "bbox_local_background_min_samples: " << bbox_local_background_min_samples << '\n';
        std::cout << "bbox_local_background_min_delta: " << bbox_local_background_min_delta << '\n';
        std::cout << "bbox_local_background_max_delta: " << bbox_local_background_max_delta << '\n';
        std::cout << "bbox_local_background_synth_margin: " << bbox_local_background_synth_margin << '\n';
        std::cout << "bbox_local_background_debug: " << bbox_local_background_debug << '\n';
        std::cout << "bbox_local_background_burn_into_synth: " << bbox_local_background_burn_into_synth << '\n';
        std::cout << "bbox_local_background_burn_cell_exclusion_scale: " << bbox_local_background_burn_cell_exclusion_scale << '\n';
        std::cout << "bbox_local_background_burn_feather_radius: " << bbox_local_background_burn_feather_radius << '\n';
        std::cout << "bbox_local_background_burn_margin: " << bbox_local_background_burn_margin << '\n';
        std::cout << "bbox_local_background_burn_global_fallback_weight: " << bbox_local_background_burn_global_fallback_weight << '\n';
        std::cout << "bbox_local_background_burn_debug: " << bbox_local_background_burn_debug << '\n';
