...
ROOT = Path('/home/puv/output_fluo/tuning_fluo_gt_20260529/gamma_25frame_original_n2v2_oldparams_r2')
...
def threshold(frame):
    if frame < 50: return 50.0
    if frame < 100: return 40.0
...
def auto():
    ensure_dirs()
    append_log('Started gamma 25-frame tuning oldparams r2', [
        ...
        f'gamma candidates: `{GAMMAS}`',
        'batches: first 200 frames as 25-frame groups; final group scores 175..194 because GT stops at 194',
        f'CPU budget: max `{MAX_WORKERS}` workers x `{THREADS}` threads = `{MAX_WORKERS*THREADS}` logical cores',
        f'disk budget: `<20GB` under `{ROOT}`',
    ])
    queue = [(b, g) for b, g in all_tasks(include_deferred=False) if (b['name'], float(g)) not in load_completed_ids()]
    ...
    append_log('Completed gamma 25-frame tuning oldparams r2', [f'metrics: `{METRICS}`', f'summary: `{SUMMARY}`', f'relation: `{RELATION}`'])
