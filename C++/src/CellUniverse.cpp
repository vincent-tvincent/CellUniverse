...
// Preprocessing orchestration lives in PreprocessingHandler. ImageHandler keeps
// the low-level image IO and contrast-processing utilities.
CellUniverse::CellUniverse(std::map<std::string, std::vector<Ellipsoid>> initialCells,
                           ...
                           int firstFrame,
                           int continueFrom)
: config(config), outputPath(outputPath), preprocessing(this->config, outputPath), firstFrame(firstFrame),
  imagePaths(imagePaths), initialCells(initialCells), continueFrom(continueFrom)
{
    ...
    if (!config.simulation.quit_after_preprocessing) {
        ...
        std::vector<cv::Mat> probe =
            preprocessing.probePreprocessedStack(imagePaths.front(), probeLog);
...
void CellUniverse::prepareFrame(int frameIndex)
{
    ...
    if (config.simulation.edge_brightness_alignment_enabled &&
        ...
        edgeTarget = edgeBrightnessAlignmentTarget;
    }

    PreprocessedFrame preprocessed = preprocessing.preprocessFrame(
        imagePaths[static_cast<size_t>(frameIndex)],
        edgeTarget,
        std::cout);
    if (config.simulation.edge_brightness_alignment_enabled &&
        !edgeBrightnessAlignmentTargetInitialized) {
        edgeBrightnessAlignmentTarget = preprocessed.sampledBackground;
    ...
    frames[frameIndex].loadImageStacks(preprocessed.stack);
    frames[frameIndex].setSignalMap(std::move(preprocessed.signalMap));
    prepareSignalCentersForFrame(frameIndex, preprocessed.stack, true);
}
...
void CellUniverse::preprocessAllFramesAlignedToMinimumBackground(bool loadIntoFrames)
{
    ...
    float resolvedEdgeTarget = 0.0f;
    std::vector<PreprocessedFrame> preprocessedFrames =
        preprocessing.preprocessBatch(imagePaths,
                                      loadIntoFrames,
                                      std::cout,
                                      &resolvedEdgeTarget);

    if (alignmentEnabled) {
        edgeBrightnessAlignmentTarget = resolvedEdgeTarget;
        edgeBrightnessAlignmentTargetInitialized = true;
    }
    ...
    for (int frameIndex = 0; frameIndex < static_cast<int>(preprocessedFrames.size()); ++frameIndex) {
        ...
        if (loadIntoFrames && !item.stack.empty()) {
            frames[static_cast<size_t>(frameIndex)].loadImageStacks(item.stack);
            frames[static_cast<size_t>(frameIndex)].setSignalMap(std::move(item.signalMap));
            prepareSignalCentersForFrame(frameIndex, item.stack, true);
        } else if (config.simulation.export_signal_debug_images && !item.stack.empty()) {
