#!/usr/bin/env python3
"""Early-prune watchdog for gamma_25frame_tuning_oldparams_r2.py.
...
REPO_DRIVER = Path('/home/puv/celluniverse/CellUniverse/C++/scripts/gamma_25frame_tuning_oldparams_r2.py')
SLEEP_SECONDS = 30
...
def should_prune(gt, batches, active):
    ...
    for frame in range(start, end + 1):
        ...
        if missing or extra or max_dist > threshold(frame):
            failures.append({
                'frame': frame,
...
def main():
    ...
    append_log('Started gamma watchdog oldparams r2', [
    ...
    while True:
        ...
        if complete and not active_controller_alive():
            append_log('Stopped gamma watchdog oldparams r2', ['controller complete and no active auto driver found'])
            return
